...
//| Indent width 2
//|
//| JEdit explicit folding - that's the three {
...
#ifdef CMT_DEBUG
# define TRACE(X...) report_debug("Comments: %s", sprintf(X))
#else
# define TRACE(X...) 0
#endif
...
LocaleString module_name = DLOCALE(0, "CMS: Comments");
...
{
  if (Site s = site()) {
    int|string n = get_db()->query("SELECT COUNT(id) as n FROM comments "
                                   "WHERE visible='y'")[0]->n;
    n = (int)n;
    string cmts = sprintf(
...
{
  path = prefix(path);
  TRACE("Clearing comments for path %O\n", path);
...
{
  path = prefix(path);
  TRACE("Setting comments invisble for path %O\n", path);
...
{
  path = prefix(path);
  TRACE("Setting comments visble for path %O\n", path);
...
#else /* Pike 7.4 cludge */
# define SMTPClient Protocols.SMTP.client
#endif

  SMTPClient cli;

  if (mixed e = catch(cli = SMTPClient(mail_server))) {
...
class TagCommentAdd // {{{
{
  ...
  class Frame
  {
    ...
    array do_return(RequestID id)
    {
      ...
      int notify = args->notify && (int)args->notify == 1;

      args->path = args->path ? prefix(args->path) :
				prefix(id->misc->localpath);

      string handle;
      if (args->identity && sizeof(args->identity))

...
# define TRACE(X...) report_debug("TABLIFY: %s", sprintf(X))
#else
# define TRACE(X...) 0
#endif
...
constant _opts = ([
  ...
  "-numstyle"        : "color:red;text-align:right",
  "numclass"         : 0,
  "-numclass"        : 0,
...
object(object) RELINK = Regexp.PCRE("(((http)|(https)|(ftp))://(" VALID_CHARS "+)"
                               "(\\." VALID_CHARS "+)+)|"
                               "(((www)|(ftp))(\\." VALID_CHARS "+)+)|"
			       "(" + EMAIL_RE + ")");
...
{
  set_module_creator("Pontus �stlund (with a little help from some original "
...
class TagTablify2
{
  ...
  string parse_indata(string content, mapping args, RequestID id)
  {
    ...
    // Look for alignment and type rules
    mapping cbs = ([ "align" : _align, "type" : _type, "caption" : _caption ]);
  ...
  string mk_table(array rows, mapping args, RequestID id)
  {
	...
	if (args["row-titles"] && col == 1) {
	  ...
	  if (args->coltype && has_index(args->coltype, col-1)) {
		...
		{
		  if ( args["-numstyle"] && !(cell_format->negnumstyle)) {
		    cell_format->style += ";" + args["-numstyle"];
		    cell_format->negnumstyle = 1;
		  }
		  if ( args["-numclass"] && !(cell_format->negnumclass)) {
...
Tekniska Verken i Link�ping AB	info@tekniskaverken.se	www.tekniskaverken.se
Pontus �stlund			spam@poppa.se		www.poppa.se
</tablify2>
</ex>
</attr>
...
<attr name='pager' value='number'><p>
 ...
 div-container with the CSS class \"tablify-pager\".</p>
</attr>
...
\"title-font\" attribute. This will run the headers (cell titles) through
<cont>gtext</cont>. You can apply all <cont>gtext</cont>-attributes by prefixing
them with \"title-\". So \"title-italic\", \"title-black\", \"title-glow\" and
so on will be copied as arguments to <cont>gtext</cont></p>
...
"type" : #"<desc type='cont'><p>
 ...
 Style with the \"numstyle\", \"-numstyle\", \"numclass\" and \"-numclass\"
 attributes.</p>
